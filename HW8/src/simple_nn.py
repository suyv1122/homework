...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(label_filename, "rb") as f:
        f.read(8)   # 跳过magic number和num_items
        y = np.frombuffer(f.read(), dtype=np.uint8)

    with gzip.open(image_filename, "rb") as f:
        f.read(16)  # 跳过magic number, num_images, rows, cols
        X = np.frombuffer(f.read(), dtype=np.uint8)
        X = X.reshape(len(y), 28 * 28).astype(np.float32)
        X /= 255.0  # 归一化到 [0,1]
...
def softmax_loss(Z, y):
    ...
    probs = softmax[np.arange(len(y)), y]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        X_batch = X[i:i + batch]
        y_batch = y[i:i + batch]
        Z = X_batch @ theta  # (batch, num_classes)
        ...
        grad = X_batch.T @ (P - Y_onehot) / len(y_batch)
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        ...
        # Forward pass
        H = X_batch @ W1  # (m, hidden_dim)
        H_relu = np.maximum(H, 0)  # ReLU
        Z = H_relu @ W2  # (m, num_classes)
        ...
        # Gradients
        grad_W2 = H_relu.T @ (P - Y_onehot) / m
        dH = (P - Y_onehot) @ W2.T
        dH[H <= 0] = 0  # ReLU backward
        grad_W1 = X_batch.T @ dH / m
